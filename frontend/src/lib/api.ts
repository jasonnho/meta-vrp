...
export const Api = {
  ...
  getJobDetail: async (job_id: string): Promise<JobDetail> => {
    ...
    if (Array.isArray(summary.vehicles)) {
      vehicles = summary.vehicles.map((v: any) => ({
        ...
        operator: v.operator
          ? { id: v.operator.id ?? v.assigned_operator_id, name: v.operator.name }
          : undefined,
        status: v.status,
    ...
    const routes: JobRoute[] = vehicles
      ...
      .map(v => ({
        ...
        sequence: (v.route as JobRouteStep[]).sort((a,b)=>a.sequence_index-b.sequence_index)
  ...
  updateOperator: (id: string, patch: Partial<Pick<Operator, "name" | "phone" | "active">>): Promise<Operator> => {
    const body: any = {};
    if (typeof patch.name === "string") body.name = patch.name;
