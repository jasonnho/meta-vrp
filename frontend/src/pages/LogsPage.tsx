...
export default function LogsPage() {
    const { status, setStatus, fromDate, setFromDate, toDate, setToDate } =
        useLogsUI();
    const [sp, setSp] = useSearchParams();
    const [sortColumn, setSortColumn] = useState<keyof HistoryItem>(
        "created_at" as keyof HistoryItem
    );
    ...
    return (
        ...
        <motion.section className="space-y-6 p-1" {...fadeIn}>
            ...
            >
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Lihat riwayat dari semua proses optimasi yang telah
                        dijalankan.
                    </p>
                </div>
                ...
                {/* Filter Area */}
                <motion.div
                    className="w-full md:w-auto md:ml-auto space-y-3"
                    {...fadeIn}
                >
                                ...
                                <div className="max-w-full overflow-x-auto">
                                    <Table className="min-w-[960px]">
                                        ...
                                        <TableCaption className="text-xs">
                                            Menampilkan {paginatedRows.length}{" "}
                                            dari {filteredAndSortedRows.length}{" "}
                                            hasil. Halaman {currentPage} dari{" "}
                                            {totalPages}.
                                        </TableCaption>
                                    </Table>
                                </div>
            ...
            <Dialog
                open={open}
                onOpenChange={(v) => (!v ? closeDetail() : void 0)}
            >
                <DialogContent className="w-[95vw] sm:max-w-3xl lg:max-w-4xl p-0 overflow-hidden max-h-[90vh] flex flex-col">
                    <DialogHeaderUI className="sticky top-0 z-10 bg-background/95 backdrop-blur px-6 py-4 border-b">
                        <div className="flex items-center justify-between">
                            <DialogTitleUI className="text-lg">
                                Detail Job Optimasi
                            </DialogTitleUI>
                            <DialogClose asChild>
                                <Button
                        ...
                        <motion.div className="px-6 py-4" {...fadeIn}>
                            {!selectedId ? (
                                ...
                                <Alert variant="destructive">
                                    ...
                                    <AlertDescription>
                                        {(detailQ.error as any)?.response?.data
                                            ?.detail ??
