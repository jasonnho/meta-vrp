...
// --- ICONS ---
import {
  Loader2, Truck, Save, ClipboardList, AlertCircle, ExternalLink, Settings2, Route, Check, Inbox,
} from "lucide-react"
...
export default function StatusPage() {
  ...
  const updateStepStatus = useMutation({
    mutationFn: (p: { jobId: string; vid: string | number; seq: number | string; status: string; reason?: string }) => {
                      ...
                      <Accordion type="multiple" className="w-full">
                        {vehicles.map((v) => {
                          ...
                          const steps: JobRouteStep[] = Array.isArray(v.route)
                            ...
                            : []
                          const vehicleProgress = calculateProgress(steps)
                            ...
                            <motion.div key={vid} variants={listItem} layout>
                              <AccordionItem value={vid}>
                                ...
                                <AccordionContent asChild>
                                  <motion.div className="px-6 py-4 border-t bg-muted/30" {...fadeIn}>
                                      ...
                                      <TabsContent value="steps" className="pt-4">
                                        <div className="rounded-md border">
                                          <ScrollArea className="h-96">
                                            <div className="p-1">
                                              {steps.length === 0 ? (
                                                <motion.div className="p-8 text-sm text-muted-foreground text-center" {...fadeIn}>
                                                ...
                                                <motion.div variants={stagger} initial="initial" animate="animate">
                                                  {steps.map((s) => {
                                                    ...
                                                    const k = `${vid}:${seq}`
                                                    const pickStatus = perStep[k]?.status ?? s.status ?? "planned"
                                                    const pickReason = perStep[k]?.reason ?? (s.reason || "")
                                                      ...
                                                      >
                                                        ...
                                                        <div className="flex items-center justify-between gap-3">
                                                          ...
                                                          {s.status === "planned" && (
                                                            <Tooltip>
                                                              <TooltipTrigger asChild>
                                                                <motion.div whileTap={{ scale: 0.98 }}>
                                                                  <Button
                                                                    ...
                                                                    variant="outline"
                                                                    className="h-8 w-8"
                                                                    disabled={!!updatingStepKey || isUpdatingThisStep}
                                                                    onClick={() =>
                                                              ...
                                                              <TooltipContent>Tandai "Visited"</TooltipContent>
                                                            </Tooltip>
                                                          )}
                                                        </div>
                                                        ...
                                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-end">
                                                          <div className="space-y-1 w-full sm:w-auto sm:min-w-[150px]">
                                                            <Label htmlFor={`s-status-${k}`} className="text-xs">
                                                              Ubah Status
                                                            </Label>
                                                            <Select
                                                              value={pickStatus}
                                                          ...
                                                          <div className="space-y-1 w-full flex-1">
                                                            <Label htmlFor={`s-reason-${k}`} className="text-xs">
                                                              Alasan (Opsional)
                                                            </Label>
                                                            <Input
                                                              id={`s-reason-${k}`}
                                                          ...
                                                          <motion.div whileTap={{ scale: 0.98 }}>
                                                            <Button
                                                              ...
                                                              variant="outline"
                                                              disabled={
                                                                !!updatingStepKey || isUpdatingThisStep || updateStepStatus.isPending
                                                              }
                                                              onClick={() =>
                                                                updateStepStatus.mutate({
                                                                  jobId: selectedJobId as string,
